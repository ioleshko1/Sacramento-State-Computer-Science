...
public class GameWorld extends Observable {
	...
	// Sound is boolean variable which captures whether the sound is on or off
	...
	public void init() {
		...
		gameObjects.add(Ant.getAnt(initial_location));
	...
	public void collisionFlag(int sequenceNumber) {
		...
		while(iterator.hasNext()) {
			...
			if(tempObject instanceof Ant) {
				...
				((Ant) tempObject).checkFlagCollision(sequenceNumber);
				
				// Check to see if the ant has reached the last flag
				if (((Ant) tempObject).getLastFlagReached() == 4) {
	...
	public void collisionFoodStation() {
		...
		// Create an iterator to keep track of the objects
		IIterator firstIterator = gameObjects.getIterator();
		
		// Here we check to see if the iterator that we are using has a next object
		while(firstIterator.hasNext()) {
			
			// If so then we create a new temporary objects and call getNext() which passes in the next object so we can store it
			GameObject tempObject = firstIterator.getNext();
			
			// Here we check to see if that object is an instance of the Ant object
			if(tempObject instanceof FoodStation) {
				
				// Capture the FoodStation object
				FoodStation food = (FoodStation) tempObject;
	...
	public void collisionSpider() {
		...
		while(iterator.hasNext()) {
			...
			if(tempObject instanceof Ant) {
				...
				if (((Ant) tempObject).getHealthLevel() <= 0) {
	...
	public void gameClockTick() {
		...
		while(iterator.hasNext()) {
			...
			if(tempObject instanceof Ant) {
				...
				((Ant) tempObject).modifyFoodLevel(getWidth(), getHeight());
				
				// Check to see if the foodLevel of the ant has reached 0
				if (((Ant) tempObject).getFoodLevel() <= 0) {
			...
			if(tempObject instanceof Spider) {
				...
				((Spider) tempObject).controlSpider(getWidth(), getHeight());
